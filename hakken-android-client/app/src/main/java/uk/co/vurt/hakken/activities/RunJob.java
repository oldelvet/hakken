...
public class RunJob extends Activity implements DatePickerDialogTools {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		previousButton.setOnClickListener(new Button.OnClickListener() {
			...
			public void onClick(View view) {
				// TODO: 
				// Save page functionality removed due to current error if
				// page is visited, as validation does not fire on this page,
				// but Finish button will still try to persist the data.
				// Look to track completed pages and only submit those that 
				// have been validated.
				//
		...
		nextButton.setOnClickListener(new Button.OnClickListener() {
			...
			public void onClick(View view) {
				if (saveAndValidatePage(jobProcessor.getCurrentPage())) {
					jobProcessor.submitPageDataItems(jobProcessor.getCurrentPage().getName());
	...
	protected void onSaveInstanceState(Bundle outState) {
		...
		// Save page paying no attention to validation errors
		// May need to address issue that if back button is then pressed 
		// and page never completed and validated, this could result in a
		// submission error.
		savePage(jobProcessor.getCurrentPage());
				...
				if (widget != null) {
					...
					if ("TEXT".equals(item.getType()) 
						...
						if (value == null || value.equals("")) {
							Log.d(TAG, "Null value for " + item.getName() + "[" + value + "]");
							String defaultValue = PageItemProcessor.getStringAttribute(item, "default");
							if (defaultValue != null) {
								Log.d(TAG, "Setting default value for " + item.getName() + "[" + defaultValue + "]");
								value = defaultValue;
	...
	protected void drawPage(boolean ignoreErrors) {
		...
		if (state == STATE_RUN) {
			...
			if (jobProcessor.isCurrentPageVisible()) {
				...
				} else if (!ignoreErrors && validationMessage.length() > 0) {
