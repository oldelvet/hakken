...
public class JobProcessor {
	...
	public static final int COLUMN_INDEX_JOB_DESCRIPTION = 9;
	public static final int COLUMN_INDEX_JOB_MODIFIED = 10;
    public static final int COLUMN_INDEX_JOB_ADHOC = 11;	
	public static final int COLUMN_INDEX_SERVER_ERROR = 12;
	...
	public JobProcessor(ContentResolver contentResolver, Uri jobUri){
		...
		if(cursor != null){
			...
			jobDefinition = new JobDefinition(
					...
					cursor.getString(COLUMN_INDEX_JOB_DESCRIPTION));
	 ...
	 */
	private Uri persistDataItem(DataItem dataItem, String goodToSubmit){
...
//		
//		// Set keyword, used for ad-hoc header on Job List page
//		values.put(Job.Definitions.NAME, "Hey Hey Hey");
//		values.put(Job.Definitions.DESCRIPTION, "Hey2 Hey2 Hey2");
//		
//		contentResolver.update(Uri.withAppendedPath(Job.Definitions.CONTENT_URI, ""+jobDefinition.getId()) , values, null, null);
	...
	private void updateJobDefinition(String keyword, String description) {
		...
		// Set keyword and / or description, used for ad-hoc header on Job List page
		if (keyword != null) {
			values.put(Job.Definitions.NAME, keyword);
		}
		if (description != null) {
			values.put(Job.Definitions.DESCRIPTION, description);			
		}
	...
	public void submitPageDataItems(String pageName){
		Log.d(TAG, "Retrieving DataItem: " + pageName);
		
		Cursor cursor = contentResolver.query(Dataitem.Definitions.CONTENT_URI,
				new String[] {Dataitem.Definitions.PAGENAME, Dataitem.Definitions.NAME, Dataitem.Definitions.TYPE, Dataitem.Definitions.VALUE },
				Dataitem.Definitions.JOB_ID + "=? " 
						+ "AND " + Dataitem.Definitions.PAGENAME + "=? ",
				new String[] { "" + getJobId(), pageName },
				null);
		
		if(cursor != null){
			cursor.moveToFirst();
			while(!cursor.isAfterLast()){
				DataItem dataItem = new DataItem(cursor.getString(0),cursor.getString(1),cursor.getString(2),cursor.getString(3));
				
				persistDataItem(dataItem, "true");
